...
const StudentsOverview = () => {
  ...
  useEffect(() => {
    selectedCohort && fetchStudentsInCohort(selectedCohort.id);
  }, []);
        ...
        {selectedStudent && (
          <div className="flex flex-col justify-center border rounded bg-slate-300 p-8">
            <div className="flex justify-evenly items-center text-xl py-4">
              ...
              <div>ID Number: {selectedStudent.id}</div>
            </div>
            ...
            <div className="flex text-xl py-4">
              <span className="pr-4 py-2">
                Absences: {selectedStudent.absent_days}
              </span>
              <span className="p-2">
                Remaining Absences: {10 - selectedStudent.absent_days}
              </span>
            </div>
              ...
              <div className="flex flex-col border">
                <div className="contact-info-top flex border">
                  <span className="text-xl border p-2">
                    Email: {JSON.parse(selectedStudent.contact_info).email}
                  </span>
                  <span className="text-xl border p-2">
                    Phone Number:{" "}
                    {JSON.parse(selectedStudent.contact_info).phone}
                  </span>
                </div>
                <div className="contact-info-bottom flex justify-center border">
                  <span className="text-xl p-2">
                    Address: {JSON.parse(selectedStudent.contact_info).address}
                  </span>
                </div>
              </div>
            </div>
