...
const CohortOverview = () => {
  ...
  const [studentsOverviewSelected, setStudentsOverviewSelected] =
    useState(true); // New state to track whether Students Overview is selected
  const [assignmentsOverviewSelected, setAssignmentsOverviewSelected] =
    useState(false); // New state to track whether Assignments Overview is selected
      ...
      <div className="flex items-center p-4 bg-slate-600">
        <ul className="flex space-x-6">
          <li onClick={() => handleTabClick("students")}>
            <button
              className={`border rounded p-4 ${
                selectedTab === "students" && studentsOverviewSelected
                  ? "bg-red-300"
                  : "bg-slate-700"
              }`}
            >
          ...
          <li onClick={() => handleTabClick("assignments")}>
            <button
              className={`border rounded p-4 ${
                selectedTab === "assignments" && assignmentsOverviewSelected
                  ? "bg-red-300"
                  : "bg-slate-700"
              }`}
            >
